...
/*Inicializa o modulo de WI-FI e tenta conecetar a rede usando SSID
e a senha e se houver falha, apos 30 segundos, desinicializa e retorna
false
*/
...
/*Callbck acionado apos resolucao DNS, armazenando, sua outra função é armazenar o ip
em uma variavel (arg) e se a resolução do DNS falhar define IP como IPADDR_NONE*/
...
static void hostname_found(const char *name, const ip_addr_t *resolved, void *arg) {
    ...
    if (resolved) {
        *result = *resolved;
        ...
        result->addr = IPADDR_NONE; 
    }
}



/*Realiza a tradução de hostname para um endereço ip*/
...
/*cria e configura uma conexão TCP com um servidor.
 define callbacks para indicar erro recepsção e etc*/
...
bool client_create(struct tcp_pcb *pcb, ip_addr_t *ip_addr, unsigned short port) {
    ...
    tcp_arg(pcb, (void *) connect_result_ptr); 
...
/*Envia string paro o servidor por meio TCP. Usa tcp_write
 para enfileirar e tcp_output para forçar o envio imediato. 
 Aguarda confirmação via callback ou timeout (5 segundos).*/
bool client_write(struct tcp_pcb *pcb, const char *data) {
    ...
    tcp_arg(pcb, (void *) write_result_ptr); 
